...
#include <cmath>
#include <chrono>
//#include "path/to/matplotlibcpp.h"
...
int main(int argc, char* argv[]) {
    ...
    // Vector to store performance data
    std::vector<std::vector<double> > performanceData;

    // Loop over the number of threads and the length of one sequence
    for (int threads = minThreads; threads <= maxThreads; ++threads) {
        for (int length = minLength; length <= maxLength; length += 100) {
            std::cout << "Threads: " << threads << ", Length: " << length << std::endl;

            auto start = std::chrono::high_resolution_clock::now();
            needlemanWunsch(seq1, seq2, scoreMatrix, threads);
            auto end = std::chrono::high_resolution_clock::now();

            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);

            performanceData.push_back({static_cast<double>(threads), static_cast<double>(length), static_cast<double>(duration.count())});
        }
    }

    std::ofstream outputFile("performance_data.csv");
    if (outputFile.is_open()) {
        outputFile << "Threads,Length,ExecutionTime" << std::endl;
        for (const auto& data : performanceData) {
            outputFile << data[0] << "," << data[1] << "," << data[2] << std::endl;
        }
        outputFile.close();
